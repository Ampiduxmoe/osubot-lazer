...
export class OsuBeatmapsDaoImpl implements OsuBeatmapsDao {
  ...
  async getByHash(
    ...
    this.recentApiRequests.add({
      ...
      appUserId: appUserId,
      target: OsuServer[api.server],
      subtarget: COMMON_REQUEST_SUBTARGETS.beatmapInfoByHash,
      count: 1,
    });
    const beatmap = await api.getBeatmapByHash(hash);
    if (beatmap === undefined) {
      return undefined;
    }
    return beatmapInternalToExternal(beatmap);
...
function beatmapInternalToExternal(beatmap: OsuBeatmapInfo): OsuBeatmap {
  return {
